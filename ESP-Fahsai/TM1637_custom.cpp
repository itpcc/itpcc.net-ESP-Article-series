...
tm1637_custom::tm1637_custom(uint8_t ClockPin, uint8_t DataPin)
{
	_clk = ClockPin;
	_dio = DataPin;
	pinMode(_dio, OUTPUT);
...
/// =======================================
void tm1637_custom::i2cStart(void)
{
    CLK_HIGH();
    DIO_HIGH();
    delayMicroseconds(2);
...
/// =============================================
void tm1637_custom::i2cAck (void)
{
    CLK_LOW();
    DIO_HIGH();
    delayMicroseconds (5); 
...
/// ========================================
void tm1637_custom::i2cStop (void) 
{
    CLK_LOW();
    delayMicroseconds (2);
    DIO_LOW();
...
void tm1637_custom::i2cWrite (uint8_t oneByte)
{
    uint8_t i;
    for (i=0; i<8; i++)
    {
...
void tm1637_custom::initDisplay(void)
{
    i2cStart();
    i2cWrite(0x40); 
    i2cAck();
...
/*  Public Methods  *******************************************/
void tm1637_custom::setBrightness(uint8_t b)
{
    _brightness = (b & 0b111);
    i2cStart();
...
void tm1637_custom::power(bool state)
{
...
void tm1637_custom::writeCustom(uint8_t digit, uint8_t value){
    i2cStart();
    i2cWrite(0xc0 | digit);
    i2cAck();
    i2cWrite(value);
...
//value [0 - 9]
void tm1637_custom::writeDigit(uint8_t digit, uint8_t value){
    writeCustom(digit, digits[value]);
}

void tm1637_custom::writeTime(uint8_t hours, uint8_t minutes, bool colon)
{
...
void tm1637_custom::writeInteger(uint16_t number){ //value [0 - 9999]
  int8_t i = 3;
     if (number == 0){
       data[3] = digits[0];
       data[1] = 0;
